...
$stmt = $pdo->prepare("SELECT * FROM Income WHERE incomeID = ? AND userID = ?");
$stmt->execute([$incomeID, $userID]);
$income = $stmt->fetch();

if (!$income) {
    die("Meow? Income record not found!");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($amount) && !empty($categoryID) && !empty($date)) {
        $sql = "UPDATE Income SET incomeCategoryID=?, amount=?, income_date=?, description=? WHERE incomeID=? AND userID=?";
...
<body>
    ...
    <div class="auth-container">
        <div class="register-wrapper yellow-wrapper">
            
            <div class="register-left yellow-theme">
                
                <div style="position: absolute; top: 20px; left: 30px; margin-top: 10px;">
                    <a href="transaction.php" class="btn-small yellow-back" style="padding: 6px 10px;">
                        <i class="fas fa-times"></i> Cancel
                    </a>
                </div>

                <img src="images/income.webp" alt="Rich Cat" class="cat-logo" style="width: 200px; border-radius: 20px;">
                
                <h1>WRONG AMOUNT? 🍋</h1>
                <p style="font-size: 2rem;">User: <strong><?php echo htmlspecialchars($userName); ?></strong></p>
                <p style="font-size: 1.5rem";>Adjust your earnings here.<br>Make it right! 💰</p>
            </div>

            <div class="register-right yellow-inputs">
                <h2 class="yellow-title">EDIT INCOME 💵</h2>

                <?php if($message): ?><p style="color:red; text-align:center;"><?php echo $message; ?></p><?php endif; ?>

                <form method="POST" id="editForm">
                    <input type="hidden" name="id" value="<?php echo $income['incomeID']; ?>">

                    <div class="input-group">
                        <div style="display:flex; justify-content:space-between;">
                            <label style="color: #F9A825;">AMOUNT (RM)</label>
                            <i class="fas fa-calculator calc-trigger" onclick="toggleKeypad()" style="cursor:pointer; color:#FBC02D; font-size:1.4rem;" title="Keypad"></i>
                        </div>
                        <input type="text" inputmode="decimal" name="amount" id="amountInput" 
                               value="<?php echo htmlspecialchars($income['amount']); ?>" 
                               style="font-size: 1.8rem; font-weight: bold; color: #F9A825;" required autocomplete="off">
                    </div>

                    <div class="num-keypad yellow-keypad" id="keypad">
                    ...
                    <div class="input-group">
                        <label style="color: #F9A825;">SOURCE</label>
                        <select name="incomeCategoryID" required>
                            <?php foreach($categories as $cat): ?>
                                <option value="<?php echo $cat['incomeCategoryID']; ?>" 
                                    <?php echo ($cat['incomeCategoryID'] == $income['incomeCategoryID']) ? 'selected' : ''; ?>>
                    ...
                    <div class="input-group">
                        <label style="color: #F9A825;">DATE</label>
                        <input type="date" name="income_date" value="<?php echo $income['income_date']; ?>" required>
                    </div>

                    <div class="input-group">
                        <label style="color: #F9A825;">DESCRIPTION</label>
                        <input type="text" name="description" value="<?php echo htmlspecialchars($income['description']); ?>" placeholder="Short description..." autocomplete="off">
                    </div>

                    <button type="submit" class="create-btn yellow-btn">
                        <i class="fas fa-check"></i> UPDATE INCOME
                    </button>
